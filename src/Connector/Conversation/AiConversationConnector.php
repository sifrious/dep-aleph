...
final readonly class AiConversationConnector implements Backfills, Connector, SyncsIncrementally
{
    public function __construct(
        private AiConversationSources $sources,
        private IngestAiConversations $ingestor,
    ) {}
    ...
    private function requiredString(OperationRequest $request, string $key): string
    {
        return $this->optionalString($request, $key)
            ?? throw new InvalidArgumentException("AI conversation parameter [{$key}] is required.");
    }
